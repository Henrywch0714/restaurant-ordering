...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    console.log('🔍 DEBUG: Checking header structure...');
    const header = document.querySelector('header');
    const h1 = document.querySelector('header h1');
    if (h1) {
        console.log('🔍 DEBUG: H1 found:', h1.textContent);
        console.log('🔍 DEBUG: H1 innerHTML:', h1.innerHTML);
        const logoImg = h1.querySelector('img.logo-icon');
        console.log('🔍 DEBUG: Logo img in H1:', logoImg);
        if (logoImg) {
            console.log('🔍 DEBUG: Logo img src:', logoImg.src);
            console.log('🔍 DEBUG: Logo img currentSrc:', logoImg.currentSrc);
        }
    }
...
function renderMenu() {
    ...
    // Only show badges if recommendations have been applied (user clicked Apply)
    const recommendationsApplied = window.recommendationsApplied || false;
    console.log('🎨 renderMenu() - Recommended items:', recommendedItems);
    console.log('🎨 renderMenu() - Recommendations applied:', recommendationsApplied);
    console.log('🎨 renderMenu() - Recommended items (as strings):', recommendedItems.map(r => String(r)));
    console.log('🎨 renderMenu() - Total menu items:', filteredItems.length);
    
    // Log ALL menu item IDs (both numeric id and mongoId if present)
    const allMenuIds = filteredItems.map(i => ({
        id: i.id,
        idType: typeof i.id,
        mongoId: i.mongoId || null,
        name: i.name
    }));
    console.log('🎨 renderMenu() - ALL menu item IDs:', allMenuIds);
    
    // Match both numeric IDs (`id`) and MongoDB ObjectId strings (`mongoId`)
    // Only show badge if recommendations are applied
    const isRecommended = (item) => {
        // Don't show badges if recommendations haven't been applied yet
        ...
        const matched = recommendedItems.some(recId => {
            const recIdStr = String(recId);
            const isMatch = idsToCheck.includes(recIdStr);
            if (isMatch) {
                console.log('✅ ID MATCH FOUND:', { idsToCheck, recId: recIdStr, name: item.name });
                return true;
            }
            return false;
        });
    ...
    // Test matching for first few items
    console.log('🔍 Testing ID matching:');
    filteredItems.slice(0, 5).forEach(item => {
        const idsToCheck = [
            typeof item.id !== 'undefined' ? String(item.id) : null,
            item.mongoId ? String(item.mongoId) : null
        ].filter(Boolean);

        const isMatch = recommendedItems.some(recId => idsToCheck.includes(String(recId)));
        console.log(`  Item "${item.name}" (IDs: ${idsToCheck.join(', ')}) - Match: ${isMatch}`);
        if (!isMatch) {
            console.log(`    ❌ No match. Recommended IDs are:`, recommendedItems.map(r => String(r)));
        }
    });
    
    // Count how many items will have badges
    const recommendedCount = filteredItems.filter(item => isRecommended(item)).length;
    console.log('🎨 renderMenu() - Items with recommendation badge:', recommendedCount);
    
    menuGrid.innerHTML = filteredItems.map(item => {
        const rec = isRecommended(item);
        const badgeHtml = rec
            ? `<span class="recommendation-badge">${window.t ? window.t('recommended') : '✓ Recommended'}</span>`
            : '';
